...
import { IHandler, IKeyNameModel } from '@infrastructure';
...
export class ChangeNodeHandler implements IHandler<ChangeNodeRequest, IFlowViewModel> {
  ...
  public handle(request: ChangeNodeRequest): IFlowViewModel {
    ...
    const outputs: INodeOutputModel<string>[] = node.outputs.map((x: IKeyNameModel) => {
      return {
        key: x.key,
        name: x.name,
  ...
  private mergeOutputsWithNumber(flow: IFlowViewModel, node: INodeViewModel, outputsNumber: number): IKeyNameModel<string>[] {
